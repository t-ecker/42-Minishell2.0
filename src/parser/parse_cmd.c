...
// until there is something else than a redir or word token parse into one cmd node that has an args list and an redir list
...
t_astNode *parse_cmd(t_parser *p)
{
	...
	command_node = create_astNode(AST_COMMAND, p);
