...
void execute_pipe_child(t_shell *shell, t_astNode *cmd, t_piper p, t_pipelineList *next)
{
	setup_child_signals();
	if (p.prev_fd != -1)
	{
...
void execute_pipe_parent(t_piper *p, int pid, t_pipelineList *next)
{
	...
	if (next)
	{
		close(p->pipe_fd[1]); //write end
		p->prev_fd = p->pipe_fd[0]; //read end
	}
	else
		p->last_pid = pid;
}
